...
- \`output/\` — 生成产物（绘图 prompt、图片、SVG）
  ...
  - \`output/diagram.svg\` — 最终可编辑 SVG（嵌入 icon）
...
6. Icon 提取与 SVG 合成（可选）：
   ...
   e. AnalyzeImage → SVG 模板（带占位符）
   e-2. RenderSvg → 渲染 PNG 预览，对比原图审核一致性
   f. AssembleSVG → 嵌入 icon 的最终 SVG
   g. Edit 微调 SVG 细节

$切记在执行生图前务必向用户确认当前方案，等用户回复确认后再执行 GenerateImage
$切记在执行生图前务必向用户确认当前方案，等用户回复确认后再执行 GenerateImage
$切记在执行生图前务必向用户确认当前方案，等用户回复确认后再执行 GenerateImage
...
`,
    ...
    },
    {
      ...
      description: '最终可编辑 SVG（嵌入 icon 图片）',
      resolver: { type: 'field', field: 'output.diagram_svg' },
    },
    {
